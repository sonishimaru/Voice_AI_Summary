...
初回起動時、**Dockに波形アイコンが表示されます**。Dockアイコンを**右クリック（またはクリック長押し）**すると、録音の一時停止・停止などの操作メニューが開きます。メニューバーにも同じアイコンが出ますが、そちらは「メニューバーに表示」のチェックを外せば消せます。
...
## 操作メニュー（Dockアイコンを右クリック）

Dockアイコンが主な操作場所です。メニューバーは項目が増えると一つひとつが小さくなり、ノッチのあるMacでは表示しきれなくなった項目が黙って隠れてしまうため、常時使う操作の置き場所としては不向きです。Dockアイコンは大きく、隠れることがなく、色と形でそのまま録音状態を示します。

| 状態 | Dockアイコン |
|---|---|
| 録音中 | **赤**いタイルに波形 |
| 一時停止中 | **オレンジ**のタイルに一時停止マーク |
| 停止中 | **グレー**のタイルに斜線入りのマイク |

色は離れた場所からでも分かり、記号は近くで見たときに区別を確定させます。どちらか一方だけに頼らないので、赤とオレンジの区別がつきにくい場合でも読み取れます。

メニューバーのアイコンも同じ内容のメニューを開きます。不要なら「メニューバーに表示」のチェックを外してください（`UserDefaults` の `showMenuBarIcon` キー。既定は表示）。Dockアイコンは常に出ます。
...
- メニューバーに表示 — メニューバーアイコンのON/OFF（Dockアイコンは常に表示）。Dockメニューにもあります
